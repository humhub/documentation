...
| # | Status | Severity | Finding |
|---|--------|----------|---------|
| E-1 | ☑ | Minor | `develop/models.md:14` — `underscorce` → `underscore`; `fieldnames andattributes` → `field names and attributes` |
| E-2 | ☑ | Minor | `admin/server-setup.md:25` — `**Datebase:**` → `**Database:**` |
| E-3 | ☑ | Minor | `admin/installation.md:98` — `sice reply` → `since reply` |
| E-4 | ☑ | Minor | `theme/structure.md:31` — `Appeareance` → `Appearance` |
| E-5 | ☑ | Minor | `develop/environment.md:27` — `Fore more infos` → `For more information` |
| E-6 | ☑ | Minor | `develop/widgets.md:136` — `Fore more complex` → `For more complex` |
| E-7 | ☑ | Minor | `develop/modules-lifecycle.md:15` — `Developement Environment` → `Development Environment` |
